...
/*
	...
	1、 STM32F407 缺省的外部晶振频率是25MHz， PLL倍频到 168MHz 作为CPU系统时钟。
		如果外部晶振频率不是 25MHz， 请修改  "stm32f4xx.h " 文件中的 "HSE_VALUE" 宏定义

		- STM32F429 缺省的外部晶振频率是8MHz， PLL倍频到 180MHz 作为CPU系统时钟。
		如果外部晶振频率不是 8MHz， 请修改  "stm32f4xx.h " 文件中的 "HSE_VALUE" 宏定义
	...
	5、 SystemCoreClock 全局变量表示系统主频（Hz），缺省是 180000000. 在C程序中可以直接使用改全局变量。
